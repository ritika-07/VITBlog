...
function createAdmin($request_values){
	...
	$val = trim($request_values['username']); // remove empty space sorrounding string
	$val = mysqli_real_escape_string($request_values['username']);
	$username = $val;
	
	$val = trim($request_values['email']); // remove empty space sorrounding string
	$val = mysqli_real_escape_string($request_values['email']);
	$email = $val;

	$val = trim($request_values['password']); // remove empty space sorrounding string
	$val = mysqli_real_escape_string($request_values['password']);
	$password = $val;

	$val = trim($request_values['passwordConfirmation']); // remove empty space sorrounding string
	$val = mysqli_real_escape_string($request_values['passwordConfirmation']);
...
function createTopic($request_values){
	...
	$val = trim($request_values['topic_name']); // remove empty space sorrounding string
	$val = mysqli_real_escape_string($conn, $request_values['topic_name']);
	$topic_name = $val;
	// create slug: if topic is "Life Advice", return "life-advice" as slug
	$string = strtolower($topic_name);
...
function updateTopic($request_values) {
	...
	$val = trim($request_values['topic_id']); // remove empty space sorrounding string
	$val = mysqli_real_escape_string($conn, $request_values['topic_id']);
	$topic_id = $val;
	// create slug: if topic is "Life Advice", return "life-advice" as slug
	$string = strtolower($topic_name);
	$slug = preg_replace('/[^A-Za-z0-9-]+/', '-', $string);
